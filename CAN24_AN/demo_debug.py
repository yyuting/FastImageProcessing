    ...
    for compiler_problem_idx in [2]:
        ...
        with tf.control_dependencies(color_features):
            with tf.variable_scope("auxiliary"):
                ...
                features_step0 = tf.parallel_stack(out_features)
                
        for k in range(100):

            arrs = sess.run([out_features[0][0], features_step0[0, 0]], feed_dict=debug[0])
            print(k)
...
def main_network(args):
    ...
    with tf.variable_scope("shader"):
        ...
        debug = [{camera_pos: np.transpose(camera_pos_vals[:1]), shader_time: time_vals[:1], h_start: np.array([-16]), w_start: np.array([-16])}]
        ...
        sess = tf.Session()
        sess.run(tf.local_variables_initializer())
        arr0, arr1 = sess.run([input_label, input_to_network[0, :, :, 0]], feed_dict=debug[0])
        skimage.io.imsave('test.png', np.clip(arr0[0], 0, 1))
        skimage.io.imsave('test1.png', np.clip(arr1, 0, 1))
        print(arr0)
        print(arr1)
        return
            ...
            if not args.train_temporal_seq:
                ...
                if not args.geometry.startswith('boids'):
                    ...
                    if args.is_train or args.collect_validate_loss:
                        ...
                        input_label = tf.stack([input_to_network[:, :, :, col] for col in color_inds], 3)
                        arr = sess.run(input_label, feed_dict=debug[0])
                        skimage.io.imsave('test.png', np.clip(arr[0], 0, 1))
                        print(arr)
                        return
