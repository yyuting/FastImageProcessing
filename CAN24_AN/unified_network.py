...
all_shaders = [('bricks', 'plane'), ('mandelbrot', 'plane'), ('mandelbulb', 'none'), ('trippy_heart', 'plane'), ('primitives_wheel_only', 'none')]
...
def main_network(args):
    ...
    if not args.use_queue:
        ...
        def load_image(ind):
            ...
            if args.temporal_texture_buffer:
                ...
                for i in range(ngts):
                    imgfile = os.path.join(queue_img_dir, '%s_ground%d%05d.png' % ('train' if args.is_train else 'test', i, ind))
                    img = np.float32(cv2.imread(imgfile, -1)) / 255.0
                    output_arr[:, :, i*3:i*3+3] = img
    ...
    def feature_reduction_layer(input_to_network, _replace_normalize_weights=None, ndims=2, shadername=''):
        with tf.variable_scope("feature_reduction" + shadername, reuse=tf.AUTO_REUSE):
            ...
            if args.add_initial_layers:
                for nlayer in range(3):
                    reduced_feat = slim.conv2d(reduced_feat, actual_initial_layer_channels, [1, 1], rate=1, activation_fn=lrelu, normalizer_fn=nm, weights_initializer=identity_initializer(allow_map_to_less=True), scope='initial_'+str(nlayer), weights_regularizer=regularizer, padding=conv_padding)          
    ...
    for shader_ind in range(len(all_shaders)):
        print('running shader %d' % shader_ind)
        
        args.shader_name = all_shaders[shader_ind][0]
        args.geometry = all_shaders[shader_ind][1]
        ...
        with tf.variable_scope("generator", reuse=tf.AUTO_REUSE):
            if args.debug_mode and args.mean_estimator:
                ...
                if not args.train_temporal_seq:
                    ...
                    input_to_network = feature_reduction_layer(input_to_network, _replace_normalize_weights=replace_normalize_weights, ndims=ndims, shadername=args.shader_name)
                    feature_reduction_tensor = input_to_network
                    
                    assert not args.learn_loss_proxy
